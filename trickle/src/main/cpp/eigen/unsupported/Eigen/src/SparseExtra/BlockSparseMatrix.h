...
// Block version of the sparse dense product
    ...
    public:
        ...
        void scaleAndAddTo(Dest &dest, const typename Rhs::Scalar &alpha) const {
            ...
            internal::sparse_time_dense_product(BlockSparseMatrixView<Lhs>(m_lhs),
                                                BlockVectorView<Lhs, Rhs>(m_lhs, m_rhs), tmpDest,
                                                alpha);
        }
    ...
    public:
        ...
        typedef Matrix<Scalar, _BlockAtCompileTime, _BlockAtCompileTime, IsColMajor ? ColMajor
                                                                                    : RowMajor> BlockScalar;
        ...
        typedef BlockSparseMatrix<Scalar, BlockSize, IsColMajor ? ColMajor
                                                                : RowMajor, StorageIndex> PlainObject;
    public:
        // Default constructor
        BlockSparseMatrix()
        ...
        inline BlockSparseMatrix &operator=(const MatrixType &spmat) {
            ...
            eigen_assert(
                    ((MatrixType::Options & RowMajorBit) != IsColMajor) && "Wrong storage order");
        ...
        void setBlockStructure(const MatrixType &blockPattern) {
            ...
            for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
                ...
                for (StorageIndex idx = 0; idx < nzBlockIdx.size(); ++idx) {
                    ...
                    if (m_blockSize == Dynamic)
                        m_blockPtr[offset] = m_blockPtr[offset - 1] +
        ...
        void setFromTriplets(const InputIterator &begin, const InputIterator &end) {
            ...
            VectorXi nz_outer(
                    m_outerBSize); // Number of nz per outer vector...for variable-size blocks
...
//        return BlockScalar::Zero(rsize, csize);
        ...
        inline BlockScalarReturnType *
        valuePtr() { return static_cast<BlockScalarReturnType *>(m_values); }
...
//    inline Scalar *valuePtr(){ return m_values; }
        ...
        inline Index blockInnerSize(Index bi) const {
            return (m_blockSize == Dynamic) ? (m_innerOffset[bi + 1] - m_innerOffset[bi])
                                            : m_blockSize;
        }

        inline Index blockOuterSize(Index bj) const {
            return (m_blockSize == Dynamic) ? (m_outerOffset[bj + 1] - m_outerOffset[bj])
                                            : m_blockSize;
        }
