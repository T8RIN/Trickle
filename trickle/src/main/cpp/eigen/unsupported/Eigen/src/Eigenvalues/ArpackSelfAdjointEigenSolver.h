...
namespace Eigen {
    ...
    ::compute(const MatrixType &A, const MatrixType &B, Index nbrEigenvalues,
        ...
        char bmat[2] = "I";
        if (eigs_sigma.substr(0, 2) == "SM" ||
            !(isalpha(eigs_sigma[0]) && isalpha(eigs_sigma[1])) || (!isBempty && !BisSPD))
            bmat[0] = 'G';
        ...
        do {
            internal::arpack_wrapper<Scalar, RealScalar>::saupd(&ido, bmat, &n, whch, &nev, &tol,
                                                                resid,
            ...
            if (ido == -1 || ido == 1) {
                ...
                if (ido == 1 && mode != 2) {
                    ...
                    if (isBempty || mode == 1)
                        Matrix<Scalar, Dynamic, 1>::Map(out2, n) = Matrix<Scalar, Dynamic, 1>::Map(
                                in, n);
                    else
                ...
                if (mode == 1) {
                    if (isBempty) {
                        ...
                        // OP = L^{-1}AL^{-T}
                        //
                        internal::OP<MatrixSolver, MatrixType, Scalar, BisSPD>::applyOP(OP, A, n,
                                                                                        in, out);
                    }
                } else if (mode == 2) {
                    if (ido == 1)
                ...
                if (isBempty || mode == 1)
                    Matrix<Scalar, Dynamic, 1>::Map(out, n) = Matrix<Scalar, Dynamic, 1>::Map(in,
                                                                                              n);
                else
                    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
                            B * Matrix<Scalar, Dynamic, 1>::Map(in, n);
        ...
        if (info == 1)
            ...
            internal::arpack_wrapper<Scalar, RealScalar>::seupd(&rvec, howmny, select,
                                                                m_eivalues.data(), v, &ldv,
                                                                &sigma, bmat, &n, whch, &nev, &tol,
                                                                resid, &ncv,
            ...
            if (info == -14)
                ...
                if (rvec) {
                    ...
                    if (mode == 1 && !isBempty && BisSPD)
                        internal::OP<MatrixSolver, MatrixType, Scalar, BisSPD>::project(OP, n, nev,
                                                                                        m_eivec.data());
    ...
    namespace internal {
        ...
        struct arpack_wrapper<float, float> {
            static inline void saupd(int *ido, char *bmat, int *n, char *which,
                ...
                ssaupd_(ido, bmat, n, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, workd,
                        workl, lworkl, info);
            }
            ...
            static inline void seupd(int *rvec, char *All, int *select, float *d,
                                     ...
                                     int *ldv, int *iparam, int *ipntr, float *workd,
                                     float *workl, int *lworkl, int *ierr) {
                sseupd_(rvec, All, select, d, z, ldz, sigma, bmat, n, which, nev, tol, resid, ncv,
                        v, ldv, iparam, ipntr,
        ...
        struct arpack_wrapper<double, double> {
            static inline void saupd(int *ido, char *bmat, int *n, char *which,
                ...
                dsaupd_(ido, bmat, n, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, workd,
                        workl, lworkl, info);
            }
            ...
            static inline void seupd(int *rvec, char *All, int *select, double *d,
                                     ...
                                     int *ldv, int *iparam, int *ipntr, double *workd,
                                     double *workl, int *lworkl, int *ierr) {
                dseupd_(rvec, All, select, d, v, ldv, sigma, bmat, n, which, nev, tol, resid, ncv,
                        v, ldv, iparam, ipntr,
                        workd, workl, lworkl, ierr);
            }
        };
        ...
        struct OP {
            static inline void
            applyOP(MatrixSolver &OP, const MatrixType &A, int n, Scalar *in, Scalar *out);
