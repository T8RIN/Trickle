...
public:
    ...
    inline const Inverse <CompleteOrthogonalDecomposition> pseudoInverse() const {
        eigen_assert(
                m_cpqr.m_isInitialized && "CompleteOrthogonalDecomposition is not initialized.");
    ...
    for (Index k = rank - 1; k >= 0; --k) {
        ...
        rhs.middleRows(rank - 1, cols - rank + 1)
                .applyHouseholderOnTheLeft(
                        matrixQTZ().row(k).tail(
                                cols - rank).transpose().template conjugateIf<!Conjugate>(),
...
template<typename _MatrixType>
template<bool Conjugate, typename RhsType, typename DstType>
void CompleteOrthogonalDecomposition<_MatrixType>::_solve_impl_transposed(const RhsType &rhs,
                                                                          DstType &dst) const {
    const Index rank = this->rank();
