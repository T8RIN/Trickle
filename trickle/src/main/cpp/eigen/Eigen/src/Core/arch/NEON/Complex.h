...
//---------- float ----------
    ...
    template<> EIGEN_STRONG_INLINE Packet1cf
    ...
    pnegate(const Packet1cf &a) { return Packet1cf(pnegate < Packet2f > (a.v)); }
    ...
    pload<Packet1cf>(const std::complex<float> *from) {
        ...
        return Packet1cf(pload < Packet2f > ((const float *) from));
    }
    ...
    ploadu<Packet1cf>(const std::complex<float> *from) {
        ...
        return Packet1cf(ploadu < Packet2f > ((const float *) from));
    }
    ...
    ploaddup<Packet1cf>(const std::complex<float> *from) { return pset1 < Packet1cf > (*from); }
...
template<> EIGEN_DEVICE_FUNC inline Packet1cf
...
return
Packet1cf(vset_lane_f32(std::imag(from[0 * stride]), tmp, 1)
);
}
...
{
EIGEN_ARM_PREFETCH(reinterpret_cast
<const float *>(addr)
); }
...
preverse(const Packet1cf &a) { return a; }
...
pcplxflip<Packet1cf>(const Packet1cf &a) { return Packet1cf(vrev64_f32(a.v)); }

template<> EIGEN_STRONG_INLINE Packet2cf

pcplxflip<Packet2cf>(const Packet2cf &a) { return Packet2cf(vrev64q_f32(a.v)); }
